...
class DiskCache:
    ...
    def __init__(self, cache_dir='F:/cache', expires=timedelta(days=30), compress=False):
        """
    ...
    def __setitem__(self, url, result):
        ...
        for style in soup.find_all('style'):
            style.decompose()
        # soup.find_all('style').decompose()
        # #soup.comment.decompose()
        strings=''.join(soup.stripped_strings).replace('\n','').replace('\t','').replace(' ','')
        seg_list=jieba.cut_for_search(strings)
...
if __name__ == '__main__':
    ...
    link_crawler('https://nba.hupu.com/', cache=DiskCache())
